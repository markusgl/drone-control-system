...
class Img_Augmentation:
    ...
    def rnd_rotation(self, image):
        ...
        rotation_matrix = cv2.getRotationMatrix2D((num_cols / 2, num_rows / 2),  np.random.randint(-25,25), 1)
...
if __name__ == "__main__":
    ...
    dir= 'D:\\tmp\\test'
    #files = augmenter.createRandomArray(dir,1)
    file= 'ursprung.jpg'
    new_file_name='1_'+file
    img= cv2.imread(os.path.join(dir,file))
    img= augmenter.add_random_shadow(img)
    cv2.imwrite(os.path.join(dir,new_file_name), img)
    ...
    #     cv2.imwrite(os.path.join(dir,file), img)
